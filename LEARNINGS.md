...
**Installed verification:** Source `f156702`, CI `34348937692` and build `34348937672` passed with 780 tests. Mac artifact `10102975970` passed checksum, strict deep codesigning, Gatekeeper notarization and stapled-ticket checks; app and libffmpeg retain Team ID `T34G959ZG8`. Installed 0.1.115 matches all 282 artifact file/symlink entries and app.asar SHA-256 `78b819f996f0334bfc9bbc81bd665a8f2eab45b70928aced7d43f1ab764b4727`. The running app restored its saved session without a login prompt, showed the composer and Restream viewer rows, passed a real Reconnect action, and received a new Twitch message. Saved Settings are unchanged, all three grants remain present, the existing window frame/icon are retained, and OBS/recording process identities stayed unchanged while the recording grew. The exact 0.1.114 bundle and private post-quit config are recoverable under Install Backups. No public test message or deliberate real-account expiry was forced; the network/expired-token sequence is proven by the executable regressions, not an overnight physical sleep test.
---
...
**Settings follow-up:** The final manual pass also found Hidden Users still directing people to the removed Hide user row action. The copy now explains saved entries and Unhide without implying Silence user hides rows; existing hidden-user data and behavior remain unchanged, with a focused regression guard.
**Lifecycle follow-up:** Live logs contained a global "WebSocket was closed before the connection was established" exception; source inspection found the receiving socket removed every listener before a deliberate stop/reconnect. Two focused tests reproduced late-error propagation, then passed after keeping the same retired-socket error sink already used by the viewer client. A separate failing timer test confirmed Kick viewer polling awaited its token probe outside its catch; moving that await into the existing catch keeps transient probe failures local. The historical generic "fetch failed" row has no stack, so it is not uniquely attributed to that polling path.
**Installed verification:** Source `e961ad4`, CI `34238053135` and signed build `34238053119` passed with 774 tests. Mac artifact `10060870949` passed checksum, strict deep codesign, Gatekeeper and stapled notarization, with app and libffmpeg Team ID `T34G959ZG8`. All 282 file/symlink manifest entries match the installed app; app.asar SHA-256 is `0bcc786d178fa9e1c7f5b77fdd3ddeca8e82998f0912ad00f67bddc620e72093`. Live MCP reports 0.1.114 and connected. A real rate-limit response remains cached when the native manual check displays its reset time, without a new checkedAt or repeated error log. The installed 460px channel panel and viewer breakdown were visually verified, complete saved Settings and grants remained present, and OBS/recording process identities stayed unchanged while the recording grew. The 0.1.113 bundle is recoverable in Install Backups. No new public test message, deliberate provider expiry, native speech injection or full public-release Squirrel install is claimed; isolated renderer/controller tests are separate from those live boundaries.
---
...
**Guard:** Four regression cases failed before their fixes; 746 tests now pass, including a simulated two-minute login, cancellation, parent close, shared login windows, actual auth-IPC ordering, deliberate sign-out during pending login, and the renderer repair button through success/cancellation without sign-out or automatic resend. Typecheck and lint have no errors. Real provider expiry remains outside the app's control; these tests do not prove that website cookies never expire. Signed installation is verified separately; the viewer feed's repeated-401 recovery issue is not changed by this fix.
**Installed verification:** Runtime source `055981c`, CI `34146339828` and signed build `34146339826` passed; arm64 artifact `10027862808` passed SHA-256, strict deep codesigning, Gatekeeper notarization and stapled-ticket validation, with app and libffmpeg Team ID `T34G959ZG8`. `/Applications/Restream Chat Plus Plus.app` matches the artifact's app.asar SHA-256 `67229642a2bd335a16a1ba192fb43c98fd32cbb6dfe82916e743c15b81a7aa4e`; live MCP reports 0.1.113, persisted auth and connected, and the real installed `window.rcpp.signInToSend()` returned true without a new login prompt or posting a message.
**Runtime boundary:** The prior 0.1.112 app is preserved in the existing Install Backups directory; RC++ reopened at the same window frame and the diagnostic panel was closed afterward. No real account was signed out to force a Google/passkey challenge; that long-login behavior is covered by executable regression tests rather than a live provider reauthentication, and OBS was not restarted or controlled (its own log records a user-initiated stream/record stop before the RC++ replacement).
---
